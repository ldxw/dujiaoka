<?php
/**
 * VpayfoxController.php
 * VMQFox支付插件
 * Author VMQFox
 * Created on 2024/1/1 00:00
 */
...
class VpayfoxController extends PayController
{
    ...
    public function gateway(string $payway, string $orderSN)
    {
        try {
            ...
            $parameter = array(
                ...
                'returnUrl' => route('vpayfox-return', ['order_id' => $this->order->order_sn]),
            ...
            // 新版API签名算法：md5('payId=' . $payId . '&param=' . $param . '&type=' . $type . '&price=' . $price . '&key=' . $key)
            $signStr = 'payId=' . $parameter['payId'] . '&param=' . $parameter['param'] . '&type=' . $parameter['type'] . '&price=' . $parameter['price'] . '&key=' . $this->payGateway->merchant_id;
            $parameter['sign'] = md5($signStr);
            $payurl = $this->payGateway->merchant_pem . 'api/order/create?' . http_build_query($parameter); //支付页面
    ...
    public function notifyUrl(Request $request)
    {
        ...
        if($payGateway->pay_handleroute != 'pay/vpayfox'){
            return 'fail';
        }
        if (!$payGateway) {
            return 'fail';
